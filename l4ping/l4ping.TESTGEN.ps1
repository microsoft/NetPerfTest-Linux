...
function test_recv {
    ...
    [string] $cmd = "./l4ping -s $g_DestIp`:$Port -R $ClientSendSize -S $ClientReceiveSize"
...
function test_send {
    ...
    [string] $cmd = "./l4ping -c $g_DestIp`:$Port -S $ClientSendSize -R $ClientReceiveSize -m $PingIterations -p `"$Percentiles`" -o $out"
...
function test_l4ping_generate {
    ...
    for ($i=0; $i -lt $g_Config.Iterations; $i++) {
        ...
        # Output send command
        test_send -Port $port -ClientReceiveSize $g_Config.ClientReceiveSize -ClientSendSize $g_Config.ClientSendSize -PingIterations $g_Config.PingIterations -Percentiles $g_Config.Percentiles -OutDir $dir -Fname "tcp.i$($g_Config.PingIterations).iter$i.csv" -SendDir $SendDir
    }
