...
function test_send_latency {
    ...
    [string] $cmd = "./secnetperf -target:$SrcIp -port:$Port -tcp:$Type -up:$Requests -down:$Size -run:$($Secs)s -rstream:1 -platency:1 $Options > $out.txt"
...
function test_send_handshakes {
    ...
    [string] $cmd = "./secnetperf -target:$SrcIp -port:$Port -tcp:$Type -conns:$Conns -run:$($Secs)s -rconn:1 -exec:maxtput -prate:1 $Options > $out.txt"
    [string] $cmdOut = (Join-Path -Path $OutDir -ChildPath "$Fname")
    Write-Output $cmd | Out-File -Encoding ascii -Append "$cmdOut.txt"
    Write-Output $cmd | Out-File -Encoding ascii -Append $g_log
    Write-Output $cmd | Out-File -Encoding ascii -Append $g_logSend
